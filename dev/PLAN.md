...
| **M5** | ▶️ | Packaging. `packaging/` builds the `.deb` and the `.rpm` — nfpm, systemd, sysusers, udev, logrotate, a vendored interpreter, and a check that runs the staged tree. Left: the builder containers, the release workflow, one line in `braemons/packages/sources.txt`, and installing it on the Pi 5 beside vstimd and triald |
