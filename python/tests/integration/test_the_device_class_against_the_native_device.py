...
from statemachined.device.statemachined_device import (
...
def supervisor(native_device):
    ...
    statemachined_device = StatemachinedDevice(
        native_device.target_url,
        bench_line_map(),
        timeout=5.0,
        on_state_visit=observed_visits.append,
    )
    statemachined_device.connect_and_greet()
    statemachined_device.observed_visits = observed_visits  # for the tests to read
    try:
        yield statemachined_device
    finally:
        statemachined_device.disconnect()
...
def test_a_board_comes_back_from_a_power_cut_running_what_it_was_told_to_run(
    ...
    first.start()
    try:
        supervisor = StatemachinedDevice(first.target_url, bench_line_map(), timeout=5.0)
        supervisor.connect_and_greet()
