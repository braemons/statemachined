...
> **The daemon is Rust now, in `daemon/`, and this document is its design
...
`PLAN.md` describes a _bridge_: a host process that translates between triald's
...
| **Compile**   | a graph authored in _names_ into the wire's _indices_. `PROTOCOL.md` puts names host-side deliberately; this is where host-side is        |
...
one job that is _only_ possible in a daemon: it has to be running and listening
...
copy of the trial-type table. What the _wire_ never carries is the trial type
...
upload and result reassembly in `tests/hardware/hardware_test_harness.py` are _"the bridge's
...
testing a copy of it."_ That is exactly what the move does, and it was the
...
// graphs/go-nogo.json — authored, stored, and served in this form
{
  ...
  "states": [
    ...
    },
    {
      ...
      "transitions": [
        { "when": { "any": ["lever_left", "lever_right"] }, "goto": "Early" },
      ],
    },
    {
      "name": "Hit",
...
**`relight_after` is the one field that is about what happens _after_ the
...
and actions immediately after it (the ordering rule _is_ the device's memory
invariant). It produces a _plan_ — an ordered list of messages — and touches no
...
host-side _and_ checks the graph against the `caps` in `hello_ack` before sending
...
> **This answers PLAN.md open question 2, "Who authors a graph?"** — _"A Python
> builder in the bridge is cheap; a visual editor in triald's web UI is not, and
> triald has a no-build-step, no-CDN rule."_ The builder is `model/graph_definition.py`, and
...
argues this before we do: _"It is a state machine that configures itself from the
trial type… Choosing a trial type and choosing a state machine are one act. That
is why the trial type is the join key."_ Selecting a trial type therefore selects
...
field this replaces — `TrialType.time_sequence`, an integer today: _"edit
sequence 3 and every trial type pointing at it silently changes meaning. Sets
were cured by naming them; time sequences want the same cure."_ A graph name is
...
|                                                 |                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The **wire** never carries a trial type         | `configure` names a _graph slot_, an index into an uploaded set. The moment firmware can branch on trial type it becomes paradigm-specific, and _"firmware stable while paradigms change"_ — the property both plans exist to protect — is gone. A slot selects a **machine**; it does not describe a **condition**, and the device still cannot tell a go trial from a catch trial |
...
before an animal is in the booth; it becomes the call that _uploads_ them too,
...
What that number does not fix is _which_ trials pay it. An upload that happens
...
`max_graphs`, the daemon checks the _summed_ usage of the set it is about to
...
Uploading _during_ a trial, into the staging buffer, and committing when the
...
_"'off' is not always 'low', so this is data rather than a zeroed word"_ — and at
...
hazard for the ESP32 — _"be careful of flash operations stalling interrupts"_ —
...
position 3, _"reported so a random timing is evidence in the record and not
merely reproducible from the seed"_, and the measured `duration_us` at position 5. Position 3 is what was asked for; position 5 is what happened.

> **This contradicts triald's `PLAN.md`**, which says _"**triald rolls the random
...
of a trial, chunked (`PROTOCOL.md` §4.3). That is the right _record_ and it stays
...
reported is a _completed visit_ that happens to carry the transition that ended
...
the _timestamp_, not the arrival time, and `entered_us` is exact. For a live UI
...
without blocking a scan, and the measured 2.5 ms per _command_ on USB CDC was
...
| `kMaxPath` today | 64, so 1 KB. `trial.h` already calls it _"about 1 KB on the reference board"_                                                                                      |
...
the raw value _and_ the host-clock estimate _and_ the estimate's uncertainty. The
...
> changes — what follows here is the _argument_ for the shape, and it stays
...
is pushed only after this, since the wiring _is_ masks over these numbers.
...
wire. The rest is the _wiring_, which after §3.3 is a standalone command rather
...
This is the same rule §4.3's `GET /api/trace/trial/{id}` already stated — _"the
daemon never sends this anywhere and never assumes anybody read it"_ — applied
...
carry each state's exit cause and measured duration, and _"the reaction time is
the last state a response left"_ is an interpretation of behaviour. It belongs
...
> loop: _"**Pull, not push.** The caller asks for a trial when it is ready, which
> keeps triald reactive and stops it becoming the session's clock."_ Under the
...
line map and the graphs are a _state-machine config_ under `/var/lib`; see §6.2.
...
comparable between trials. triald can only ask vstimd _"was there frame loss
during trial 42"_ if trial 42's window arrives in a clock vstimd shares. Hand
...
sketch got wrong: the device's `seq` counts visits within a _run_ and restarts
...
within a _run_ and restarts at zero every trial, so it cannot address a position
in a log that spans a session. Both are carried in every entry, and `seq` is what
a _dropped_ visit is detected with, which is the job it can do.
...
<script
  type="module"
  src="http://rig.local:8081/elements/statemachined.js"
></script>
...
- **Custom elements are the only interop layer the three UIs share.** vstimd is
  ...
  attribute-only support suffices. A shared _framework_ would mean triald grows a
...
`packaging/avahi/vstimd.service.tmpl`, with a stable `id=` TXT record _"so
clients can match a device across name collisions"_. statemachined advertises
...
same material twice: a state-machine config _is_ a line map plus a set of
...
**Serial monitor** are three views of the single question _what is this rig
doing_, which is why somebody watching a rig had to keep switching between them.
...
| **Setup**  | what this rig is set up to do — the config on top, and under it the two things that are _in_ the config: the wiring, and the paradigms             |
...
and which one applies is decided by _who caused the repaint_:
...
panel, and the test asserts the field is the _same element_ after a poll.
...
- **Named `braemons-statemachined`,** like every package in the braemons
  ...
  Nothing _inside_ the package carries it — the binary, the unit, the user and
...
stable symlink `/dev/braemons/statemachined0`. That also solves _which port_ on a
...
The tag-driven `release.yml` builds the packages _and_ `make image`, and attaches
...
`braemons/packages/sources.txt`, and all four daemons have theirs:
...
|            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **M4a** ✅ | **The move, and nothing else.** `tools/bringup/` → `daemon/`, `statemachined_bringup` → `statemachined` with the wire under `device/`, `wire.py` made standalone with golden vectors against the emulator's copy _and_ against `crc16.cpp`. The graph upload and result reassembly left `tests/hardware/hardware_test_harness.py` for `device/graph_set_upload.py` and `device/trial_result_reassembly.py`, so the hardware suite now tests the daemon's codec rather than a copy of it. `make bringup` and `make test-hardware` unchanged in behaviour; `make test-daemon` is new and is in `make ci` and CI                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
reply. What is new is that the daemon is a _continuous_ source of commands
...
carried had a _second_ `TrialRunner`, so the bench image paid for the path twice
...
because §3.2 was costing _bigger pools_, and M4c deliberately did not grow them.
...
pyserial opens a _path_, so the two ends of a pty pair cannot both be reached
...
implemented now, in the firmware, with the _inverse_ of the patch stored so a
...
_One lock, held by everything that talks to the device._ A serial link is not
...
_A thread that reads the link when nobody asked it to._ A result and the `visit`
...
the device's but whose _arrival_ is whenever somebody next asked is not a trace.
...
1. **The bench CLI's name.** triald has `triald` + `trialctl`; vstimd has one
   ...
   that the bench tool _"is not the bridge"_, and folding it into the daemon
...
10. **How big the shared pools should be.** Answered halfway. M4b and M4c are
    ...
    path. What is _not_ done is growing the pools: the set holds twenty slots
    ...
    refactor. Note `TransitionState` is 12 B per transition _per machine_, and a
    bench build has two.
