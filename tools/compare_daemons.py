...
SCRIPT = [
    ...
    ("autorun with no board", lambda c: c.read_autorun()),
    ("saving with no board", lambda c: c.save_settings()),
...
TRIALS = [
    ...
    # The board on its own: recorded, never started, so nothing below races a
    # board arming trials of its own accord.
    ("autorun, as the board has it", lambda c: c.read_autorun()),
    ("autorun with no graph named and none active",
     lambda c: c.write_autorun(True, start_now=False)),
    ("autorun on a graph the set has not got",
     lambda c: c.write_autorun(True, graph_name="go-nogo", start_now=False)),
    ("autorun on a negative seed",
     lambda c: c.write_autorun(True, graph_name="timed-walk", seed=-42, start_now=False)),
    ("autorun, recorded and not started",
     lambda c: c.write_autorun(True, graph_name="timed-walk", seed=7, first_trial_id=100,
                               cap_milliseconds=5000, start_now=False)),
    ("autorun, read back", lambda c: c.read_autorun()),
    ("saving the board's settings", lambda c: c.save_settings()),
    ("saving them again, unchanged", lambda c: c.save_settings()),
    ("autorun off", lambda c: c.write_autorun(False)),
    ("what the trace says about it", lambda c: [
        {"kind": e.kind, "payload": e.payload} for e in c.read_trace().entries[-6:]]),
